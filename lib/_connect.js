...
async function stripeConnectStatus(req, res, ctx) {
  ...
  if (account?.stripe_account_id && ctx.stripeKey) {
    try {
      ...
      const newStatus = stripeAcct.payouts_enabled && stripeAcct.details_submitted ? 'active'
                      : stripeAcct.details_submitted ? 'onboarding'
                      : 'pending';
      if (newStatus !== account.status
          || !!stripeAcct.payouts_enabled !== !!account.payouts_enabled
          || !!stripeAcct.details_submitted !== !!account.details_submitted) {
