...
const { sbHeaders, toNumber, estimateRouteKm, roleIn } = require('./_lib');
...
async function rideCreate(req, res, ctx, body) {
  ...
  while (maxRetries-- > 0) {
    ...
    const colMatch = errMsg.match(/column ["']?(\w+)["']?\s+(does not exist|not found|of relation)/i)
                  || errMsg.match(/could not find the ["']?(\w+)["']?\s+column/i)
                  || errMsg.match(/['"](\w+)['"] does not exist/i);
    if (colMatch && colMatch[1] && attempt[colMatch[1]] !== undefined) {
