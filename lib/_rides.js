...
async function rideBook(req, res, ctx, body) {
  ...
  const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(ride_id)}&select=id,driver_id,available_seats,total_seats,status,cost_per_km,total_distance_km,accepts_pets,accepts_large_luggage,commission_free`, {
    headers: sbHeaders(ctx.sbKey),
  });
  ...
  return res.status(200).json({ success: true, booking: saved, price_breakdown: price, reused_cancelled_booking: reusedCancelledBooking });
}
...
async function rideSafetyCodeStatus(req, res, ctx, body) {
  ...
  const out = { has_profile_code: !!profileHash };
