...
function calcRidePrice({ totalDistanceKm, passengerDistanceKm, costPerKm, hasLuggage, hasPet, extraStops, detourKm, seats, confirmedPassengers, rideSettings, commissionFree }) {
  const s = rideSettings || {};
...
async function rideBook(req, res, ctx, body) {
  ...
  const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${ride_id}&select=id,driver_id,available_seats,status,cost_per_km,total_distance_km,accepts_pets,accepts_large_luggage,commission_free`, {
    headers: sbHeaders(ctx.sbKey),
  });
...
async function refundRideBooking(ctx, booking) {
  ...
  if (!ctx.stripeKey) return { action: 'stripe_non_configure' };

  const pi = await stripeGetPaymentIntent(piId, ctx.stripeKey);
  if (!pi) return { action: 'pi_introuvable' };

  if (pi.status === 'requires_capture') {
    await fetch(`https://api.stripe.com/v1/payment_intents/${encodeURIComponent(piId)}/cancel`, {
      method: 'POST',
      headers: {
        Authorization: 'Bearer ' + ctx.stripeKey,
        'Stripe-Version': STRIPE_VERSION,
        'Idempotency-Key': `ride-cancel-pi-${booking.id}`,
      },
    }).catch(() => {});
  ...
  if (pi.status === 'succeeded') {
    ...
    await fetch('https://api.stripe.com/v1/refunds', {
      ...
      headers: {
        ...
        'Content-Type': 'application/x-www-form-urlencoded',
        'Stripe-Version': STRIPE_VERSION,
        'Idempotency-Key': `ride-refund-${booking.id}`,
      },
      body: params.toString(),
    }).catch(() => {});
...
async function transferToDriverConnect(ctx, driverId, amountCents, currency, sourceCharge, bookingId) {
  if (!ctx.stripeKey || !driverId || !(amountCents > 0)) return null;
  ...
  if (!acct || !acct.stripe_account_id || acct.status !== 'active') return null;
  const params = new URLSearchParams({
    amount: String(amountCents),
    currency: currency || 'cad',
  ...
  const tr = await fetch('https://api.stripe.com/v1/transfers', {
    ...
    headers: {
      ...
      'Content-Type': 'application/x-www-form-urlencoded',
      'Stripe-Version': STRIPE_VERSION,
      'Idempotency-Key': 'ride-comp-tr-' + bookingId,
    },
    body: params.toString(),
  }).catch(() => ({ ok: false }));
...
async function recordRideLedger(ctx, { userId, type, montant, description, metadata, piId }) {
  if (!(montant > 0)) return;
  await fetch(`${ctx.sbUrl}/rest/v1/transactions`, {
    method: 'POST',
    headers: sbHeaders(ctx.sbKey),
...
async function rideCancel(req, res, ctx, body) {
  ...
  if (booking_id) {
    ...
    const policy     = getPassengerCancelPolicy(settings, hoursUntil);
    ...
    if (isReal && ctx.stripeKey) {
      ...
      if (pi && pi.status === 'requires_capture') {
        if (retainedCents <= 0) {
          await fetch(`https://api.stripe.com/v1/payment_intents/${encodeURIComponent(piId)}/cancel`, {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + ctx.stripeKey, 'Stripe-Version': STRIPE_VERSION, 'Idempotency-Key': `ride-cxl-cancel-${b.id}` },
          }).catch(() => {});
          ...
          const capRes = await fetch(`https://api.stripe.com/v1/payment_intents/${encodeURIComponent(piId)}/capture`, {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + ctx.stripeKey, 'Content-Type': 'application/x-www-form-urlencoded', 'Stripe-Version': STRIPE_VERSION, 'Idempotency-Key': `ride-cxl-cap-${b.id}` },
            body: new URLSearchParams({ amount_to_capture: String(retainedCents) }).toString(),
          }).catch(() => ({ ok: false }));
          const capPi = capRes.ok ? await capRes.json().catch(() => ({})) : {};
          stripeAction = capRes.ok ? 'capture_partielle' : 'echec_capture';
          if (capRes.ok && driverCompCents > 0 && capPi.latest_charge && ride?.driver_id) {
            const trId = await transferToDriverConnect(ctx, ride.driver_id, driverCompCents, currency, capPi.latest_charge, b.id);
            if (!trId) stripeAction += '_conducteur_en_attente';
          }
        }
        ...
        if (refundCents > 0) {
          await fetch('https://api.stripe.com/v1/refunds', {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + ctx.stripeKey, 'Content-Type': 'application/x-www-form-urlencoded', 'Stripe-Version': STRIPE_VERSION, 'Idempotency-Key': `ride-cxl-refund-${b.id}` },
            body: new URLSearchParams({ payment_intent: piId, amount: String(refundCents), reason: 'requested_by_customer' }).toString(),
          }).catch(() => {});
        }
        stripeAction = 'remboursement_partiel';
        if (driverCompCents > 0 && ride?.driver_id) {
          const trId = await transferToDriverConnect(ctx, ride.driver_id, driverCompCents, currency, pi.latest_charge, b.id);
          if (!trId) stripeAction += '_conducteur_en_attente';
        }
      } else if (pi) {
    ...
    await recordRideLedger(ctx, { userId: null,                type: 'fond_securite_covoiturage',  montant: fundCents / 100,       description: `Fond de sécurité — annulation ${policy.tier}`,        metadata: meta, piId });
    await recordRideLedger(ctx, { userId: ride?.driver_id || null, type: 'dedommagement_covoiturage', montant: driverCompCents / 100, description: `Dédommagement annulation passager (${policy.tier})`, metadata: meta, piId });
  ...
  if (ride_id) {
    ...
    for (const b of bookings) {
      ...
      try {
        ...
        action = 'erreur_remboursement';
      }
      // Marque la réservation comme annulée par le chauffeur
      await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${b.id}`, {
        method: 'PATCH',
        headers: sbHeaders(ctx.sbKey),
