...
// Triggered manuellement (admin) ou via cron. Délai grace par défaut : 4h après departure_time.
async function rideCaptureEligible(req, res, ctx, body) {
  ...
  // Admin OU le passager OU le conducteur peuvent déclencher leur propre capture
  const isAdmin = ctx.profile?.role === 'admin';
  const graceHours = Math.max(0, Math.min(168, Number(body?.grace_hours || 4)));

  // Si booking_id fourni : traite UN booking spécifique
  // Sinon : trouve tous les éligibles (admin only)
  let bookings = [];
  if (body?.booking_id) {
    const b = await fetchRideBookingForPayment(ctx, body.booking_id);
    if (!b) return res.status(404).json({ error: 'Reservation introuvable' });
    if (!isAdmin && b.passenger_id !== ctx.session.id && b.driver_id !== ctx.session.id) {
      return res.status(403).json({ error: 'Non autorisé' });
    }
    bookings = [b];
  } else {
    if (!isAdmin) return res.status(403).json({ error: 'Admin requis pour lister tous les éligibles' });
    // Trouve les bookings : status='confirme', trajet passé depuis +Xh, pas encore capturé
    const cutoffIso = new Date(Date.now() - graceHours * 3600 * 1000).toISOString();
    const url = `${ctx.sbUrl}/rest/v1/ride_bookings?status=eq.confirme&paid_at=is.null` +
                `&select=id,ride_id,passenger_id,driver_amount,total_passenger,stripe_payment_intent,confirmed_at` +
                `&order=confirmed_at.asc&limit=100`;
    const r = await fetch(url, { headers: sbHeaders(ctx.sbKey) });
    const all = r.ok ? await r.json() : [];
    // Filtre par trajet passé (cross-join avec rides pour avoir departure_time)
    if (all.length) {
      const rideIds = [...new Set(all.map(b => b.ride_id))];
      const rideRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=in.(${rideIds.join(',')})&select=id,departure_time,driver_id`, {
        headers: sbHeaders(ctx.sbKey)
      });
      const rides = rideRes.ok ? await rideRes.json() : [];
      const rideById = Object.fromEntries(rides.map(r => [r.id, r]));
      bookings = all.filter(b => {
        const r = rideById[b.ride_id];
        if (!r?.departure_time) return false;
        const dep = new Date(r.departure_time).getTime();
        return dep + graceHours * 3600 * 1000 < Date.now();
      }).map(b => ({ ...b, _ride: rideById[b.ride_id] }));
    }
  }

  if (!bookings.length) {
    return res.status(200).json({ captured: [], skipped: [], total: 0, message: 'Aucune réservation éligible' });
  }

  const captured = [], skipped = [], errors = [];
  for (const b of bookings) {
    try {
      const result = await captureOneRideBooking(ctx, b);
      if (result.ok) captured.push({ booking_id: b.id, amount: result.amount, transfer_id: result.transferId });
      else skipped.push({ booking_id: b.id, reason: result.reason });
    } catch (e) {
      errors.push({ booking_id: b.id, error: e.message });
    }
  }
  return res.status(200).json({ total: bookings.length, captured, skipped, errors });
}

async function captureOneRideBooking(ctx, booking) {
  const piId = booking.stripe_payment_intent;
  if (!piId) return { ok: false, reason: 'No PaymentIntent' };

  // Détection placeholder
  if (/pi_X{4,}/i.test(piId) || /pi_TEST/i.test(piId) || piId.length < 20) {
    // Marque payé sans Stripe (livraison test)
    await patchRideBookingPayment(ctx, booking.id, {
      status: 'paye',
      paid_at: new Date().toISOString()
    });
    return { ok: true, amount: booking.driver_amount, transferId: null, note: 'placeholder PI, manual mark' };
  }

  // 1. Capture le PaymentIntent
  const captureRes = await fetch(`https://api.stripe.com/v1/payment_intents/${piId}/capture`, {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + ctx.stripeKey,
      'Content-Type': 'application/x-www-form-urlencoded',
      'Idempotency-Key': `ride-capture-${booking.id}`
    }
  });
  const captureData = await captureRes.json();
  if (!captureRes.ok) {
    if (captureData.error?.code === 'payment_intent_unexpected_state') {
      // Déjà capturé : on continue avec transfer
      const piRes = await fetch(`https://api.stripe.com/v1/payment_intents/${piId}`, {
        headers: { 'Authorization': 'Bearer ' + ctx.stripeKey }
      });
      const pi = await piRes.json();
      return await transferRideToDriver(ctx, booking, pi);
    }
    return { ok: false, reason: captureData.error?.message || 'Capture failed' };
  }

  return await transferRideToDriver(ctx, booking, captureData);
}

async function transferRideToDriver(ctx, booking, paymentIntent) {
  const driverAmountCents = Math.round((booking.driver_amount || 0) * 100);
  const chargeId = paymentIntent.latest_charge;

  // Récupère le compte Stripe Connect du conducteur
  const rideRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${booking.ride_id}&select=driver_id`, {
    headers: sbHeaders(ctx.sbKey)
  });
  const ride = rideRes.ok ? (await rideRes.json())[0] : null;
  if (!ride?.driver_id) return { ok: false, reason: 'Driver introuvable' };
  ...
  let transferId = null;
  if (acct?.stripe_account_id && acct.status === 'active' && chargeId && driverAmountCents > 0) {
    const params = new URLSearchParams({
      amount: String(driverAmountCents),
      currency: paymentIntent.currency || 'cad',
      destination: acct.stripe_account_id,
      source_transaction: chargeId,
      description: `PorteàPorte covoit booking ${booking.id.slice(0,8)}`,
      'metadata[booking_id]': booking.id,
      'metadata[ride_id]': booking.ride_id
    });
    const tr = await fetch('https://api.stripe.com/v1/transfers', {
      ...
      headers: {
        'Authorization': 'Bearer ' + ctx.stripeKey,
        'Content-Type': 'application/x-www-form-urlencoded',
        'Idempotency-Key': `ride-transfer-${booking.id}`
      },
      body: params
    });
    ...
    if (tr.ok) {
      ...
      console.warn('[ride-capture] Transfer échec:', trData.error?.message);
    }
  }
  ...
  return { ok: true, amount: booking.driver_amount, transferId, payment_intent: paymentIntent.id };
}
...
module.exports = {
  ...
  rideDriverProfile, rideCreate, rideSearch, rideDetail, rideBook, ridePaymentCreate, ridePaymentSync, rideCancel, rideCaptureEligible,
