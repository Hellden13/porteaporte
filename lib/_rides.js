...
async function rideSearch(req, res, ctx, body) {
  ...
  const [profilesRes, driverProfilesRes, reviewsRes] = await Promise.all([
    fetch(`${ctx.sbUrl}/rest/v1/profiles?id=in.(${driverIds.join(',')})&select=id,prenom,driver_status,score_confiance,created_at,identity_verified`, { headers: sbHeaders(ctx.sbKey) }),
