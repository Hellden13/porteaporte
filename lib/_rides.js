  ...
  if (!bRes.ok) {
    ...
    if (code === '23505' || /duplicate key/i.test(msg)) {
      ...
      if (/ride_id.*passenger_id/i.test(bData?.details || '')) {
        ...
        const existRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(ride_id)}&passenger_id=eq.${encodeURIComponent(ctx.session.id)}&select=id,status,created_at`, {
          headers: sbHeaders(ctx.sbKey)
        });
