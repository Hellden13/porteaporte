...
async function fetchRideBookingSafety(ctx, bookingId) {
  ...
  const r = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(safeId)}&select=id,ride_id,passenger_id,status,total_passenger,driver_amount,platform_fee,stripe_payment_intent,payment_status,paid_at,updated_at,safety_code_hash,safety_code_custom,safety_alert_triggered&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const rows = r.ok ? await r.json().catch(() => []) : [];
...
async function rideComplete(req, res, ctx, body) {
  ...
  if (!await compareSafetyCode(code, hash)) return res.status(403).json({ error: 'Code invalide' });
