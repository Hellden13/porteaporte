...
const { sbHeaders, toNumber, estimateRouteKm, roleIn, safeIds, alertAdmin } = require('./_lib');
const bcrypt = require('bcryptjs');

// ─── Constantes tarifaires ────────────────────────────────────────────────────
const RIDE_COST_PER_KM       = 0.35;
const RIDE_PLATFORM_FEE      = 1.50; // frais fixe par siege reserve (palier bas)
const RIDE_PLATFORM_FEE_HIGH = 3.00; // palier haut par siege
const RIDE_FEE_THRESHOLD     = 15.00; // sous-total passager avant frais plateforme
const RIDE_PLATFORM_FEE_MIN  = 1.00; // plancher absolu
const RIDE_MIN_CHARGE        = 2.50; // montant minimum facturé au passager (couvre frais Stripe)
...
function ridePlatformFeePerSeat(subtotalBeforePlatform, rideSettings) {
  const s = rideSettings || {};
  const low = Math.max(RIDE_PLATFORM_FEE_MIN, toNumber(s.ride_platform_fee, RIDE_PLATFORM_FEE));
  const high = Math.max(low, toNumber(s.ride_platform_fee_high, RIDE_PLATFORM_FEE_HIGH));
  const threshold = Math.max(0, toNumber(s.ride_fee_threshold, RIDE_FEE_THRESHOLD));
  return Number(subtotalBeforePlatform) >= threshold ? high : low;
}
...
function calcRidePrice({ totalDistanceKm, passengerDistanceKm, costPerKm, hasLuggage, hasPet, extraStops, detourKm, seats, totalSeats, confirmedPassengers, rideSettings, commissionFree }) {
  ...
  const subtotalBeforePlatform = commissionBase + luggageFee;
  const platformFeePerSeat = ridePlatformFeePerSeat(subtotalBeforePlatform, s);
  ...
  const totalPassengerRaw = Math.round((commissionBase + platformFee + luggageFee) * 100) / 100;
  const totalPassenger = Math.max(RIDE_MIN_CHARGE, totalPassengerRaw);
  ...
  return {
    ...
    platformFee, platformFeePerSeat, rideFeeThreshold: Math.max(0, toNumber(s.ride_fee_threshold, RIDE_FEE_THRESHOLD)), driverAmount, totalPassenger,
...
async function stripeGetConnectAccount(accountId, stripeKey) {
  if (!accountId || !stripeKey) return null;
  const r = await fetch(`https://api.stripe.com/v1/accounts/${encodeURIComponent(accountId)}`, {
    headers: {
      Authorization: 'Bearer ' + stripeKey,
      'Stripe-Version': STRIPE_VERSION,
    },
...
async function patchRideTransactionMetadata(ctx, piId, metadataPatch, patch = {}) {
  if (!piId) return false;
  const get = await fetch(
    `${ctx.sbUrl}/rest/v1/transactions?stripe_payment_intent=eq.${encodeURIComponent(piId)}&select=id,metadata&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const rows = get.ok ? await get.json().catch(() => []) : [];
  const tx = rows[0];
  const nextMetadata = { ...(tx?.metadata || {}), ...(metadataPatch || {}) };
  return patchRideTransaction(ctx, piId, { ...patch, metadata: nextMetadata });
}
...
async function sendBookingConfirmationEmails(ctx, booking) {
  ...
  // 2) Email passager
  if (pax?.email) {
    emailJobs.push(fetch(notifierUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(secret ? { 'x-internal-notifier-secret': secret } : {}) },
      body: JSON.stringify({
  ...
  if (emailJobs.length) {
    ...
    if (failed.length) console.warn('[rideBookingEmails] echec notifications:', failed.length);
  }
}

function rideNotifierUrl() {
  return `${process.env.VERCEL_URL ? 'https://' + process.env.VERCEL_URL : 'https://porteaporte.site'}/api/notifier`;
}

async function postRideNotifier(type, data) {
  const secret = process.env.INTERNAL_API_SECRET || '';
  return fetch(rideNotifierUrl(), {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(secret ? { 'x-internal-notifier-secret': secret } : {})
    },
    body: JSON.stringify({ type, data })
  });
}

async function sendRideBookingToDriverEmail(ctx, booking) {
  const rideRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(booking.ride_id)}&select=*`, {
    headers: sbHeaders(ctx.sbKey)
  });
  const ride = rideRes.ok ? (await rideRes.json().catch(() => []))[0] : null;
  if (!ride) return;

  const [paxRes, drvRes] = await Promise.all([
    fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(booking.passenger_id)}&select=email,prenom,nom,telephone`, { headers: sbHeaders(ctx.sbKey) }),
    fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ride.driver_id)}&select=email,prenom,nom,telephone`, { headers: sbHeaders(ctx.sbKey) })
  ]);
  const pax = paxRes.ok ? (await paxRes.json().catch(() => []))[0] : null;
  const drv = drvRes.ok ? (await drvRes.json().catch(() => []))[0] : null;
  const driverEmail = drv?.email || process.env.ADMIN_ALERT_EMAIL || process.env.ADMIN_EMAIL || '';
  ...
  await postRideNotifier('ride_booking_to_driver', {
    booking_id: booking.id,
    ville_depart: ride.start_city,
    ville_arrivee: ride.end_city,
    departure_time: departureStr,
    seats: booking.seats_reserved || 1,
    pickup_label: booking.pickup_city || ride.start_city,
    dropoff_label: booking.dropoff_city || ride.end_city,
    ...
    has_pet: !!booking.has_pet,
    special_requests: booking.special_requests || '',
    total_price: (Number(booking.total_passenger || 0)).toFixed(2),
    driver_amount: (Number(booking.driver_amount || 0)).toFixed(2),
  });
}

async function sendRideDriverCompletedPassengerEmail(ctx, booking) {
  const rideRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(booking.ride_id)}&select=*`, {
    headers: sbHeaders(ctx.sbKey)
  });
  const ride = rideRes.ok ? (await rideRes.json().catch(() => []))[0] : null;
  ...
  const [paxRes, drvRes] = await Promise.all([
    fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(booking.passenger_id)}&select=email,prenom,nom`, { headers: sbHeaders(ctx.sbKey) }),
    fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ride.driver_id)}&select=email,prenom,nom`, { headers: sbHeaders(ctx.sbKey) })
  ]);
  const pax = paxRes.ok ? (await paxRes.json().catch(() => []))[0] : null;
  const drv = drvRes.ok ? (await drvRes.json().catch(() => []))[0] : null;
  if (!pax?.email) return;
  ...
  await postRideNotifier('ride_driver_completed_to_passenger', {
    booking_id: booking.id,
    passenger_email: pax.email,
    passenger_name: pax.prenom || 'Passager',
    driver_name: drv?.prenom || 'Conducteur',
    ville_depart: ride.start_city,
    ville_arrivee: ride.end_city,
    departure_time: departureStr,
...
async function sendRideMeetupPassengerEmail(ctx, { booking, ride, passenger, driver, message }) {
  const to = String(passenger?.email || '').trim();
  const key = process.env.SENDGRID_API_KEY || '';
  const fromEmail = process.env.FROM_EMAIL || process.env.SENDGRID_FROM_EMAIL || process.env.ADMIN_EMAIL || '';
  const fromName = process.env.FROM_NAME || 'PorteàPorte';
  if (!to || !key || !fromEmail) return;

  const startLabel = [ride.start_city, ride.start_sector].filter(Boolean).join(' · ') || ride.start_city || '?';
  const endLabel = [ride.end_city, ride.end_sector].filter(Boolean).join(' · ') || ride.end_city || '?';
  const departureStr = ride.departure_time ? new Date(ride.departure_time).toLocaleString('fr-CA', {
    weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
    hour: '2-digit', minute: '2-digit', timeZone: 'America/Toronto'
  }) : 'Date à confirmer';
  ...
  await fetch('https://api.sendgrid.com/v3/mail/send', {
    ...
    body: JSON.stringify({
      ...
      subject: `📍 Point de rencontre — votre trajet ${ride.start_city || '?'} → ${ride.end_city || '?'} le ${departureStr}`,
      content: [{
        type: 'text/html',
        value: `
          <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#05080c;color:#f7f8fb;border-radius:12px;padding:28px">
            <div style="color:#5dbfff;font-weight:900;font-size:.8rem;letter-spacing:.1em;margin-bottom:12px">PORTEÀPORTE — POINT DE RENCONTRE</div>
            <p>Bonjour ${htmlEscape(passenger?.prenom || 'Passager')},</p>
            <p>${htmlEscape(driver?.prenom || 'Votre chauffeur')} vous a envoyé un message concernant votre point de rencontre :</p>
            <div style="background:rgba(93,191,255,.08);border:1px solid rgba(93,191,255,.3);border-radius:10px;padding:16px;margin:18px 0;font-size:1rem;line-height:1.5">"${htmlEscape(message)}"</div>
            <p><strong>Trajet :</strong> ${htmlEscape(startLabel)} → ${htmlEscape(endLabel)}<br>
            <strong>Départ :</strong> ${htmlEscape(departureStr)}</p>
            <p style="color:#a8b0ba;margin-top:22px">— L'équipe PorteàPorte</p>
          </div>`
      }]
    })
  });
...
async function rideCreate(req, res, ctx, body) {
  ...
  const {
    ...
    start_sector, end_sector, start_meeting_point, end_meeting_point,
  ...
  const payload = {
    ...
    start_meeting_point: (start_meeting_point || body.pickup_safe_label) ? String(start_meeting_point || body.pickup_safe_label).trim().slice(0, 240) : null,
    ...
    end_meeting_point: (end_meeting_point || body.dropoff_safe_label) ? String(end_meeting_point || body.dropoff_safe_label).trim().slice(0, 240) : null,
  ...
  const optionalFields = ['commission_free', 'stop_points', 'package_max_kg', 'package_max_dim_cm', 'accepts_packages', 'smoking_policy', 'music_policy', 'chat_policy', 'ac_available', 'women_only', 'child_seat_available', 'accessible', 'personal_rules', 'cost_per_km', 'total_distance_km', 'flexibility_minutes', 'is_recurring', 'recurrence_days', 'start_sector', 'end_sector', 'start_meeting_point', 'end_meeting_point', 'start_lat', 'start_lng', 'end_lat', 'end_lng', 'return_departure_time', 'is_return_trip', 'trunk_size', 'accepts_pets', 'accepts_large_luggage', 'accepts_extra_stops'];
  let attempt = { ...payload };
  let r, data;
...
// Modifier un trajet publie sans casser les reservations existantes.
// Regle prudente: aucune modification si une reservation active existe.
async function rideUpdate(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });

  const rideId = safeRestId(body.ride_id || body.id);
  if (!rideId) return res.status(400).json({ error: 'ride_id requis ou invalide' });

  const rideRes = await fetch(
    `${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=id,driver_id,status`,
    { headers: sbHeaders(ctx.sbKey) }
  );
  const rideRows = rideRes.ok ? await rideRes.json().catch(() => []) : [];
  const ride = Array.isArray(rideRows) ? rideRows[0] : null;
  if (!ride) return res.status(404).json({ error: 'Trajet introuvable' });

  const isAdmin = roleIn(ctx.profile, ['admin']);
  if (!isAdmin && ride.driver_id !== ctx.session.id) {
    return res.status(403).json({ error: 'Seul le conducteur du trajet peut le modifier' });
  }

  const closedStatuses = ['annule', 'annule_chauffeur', 'termine', 'complete', 'completed', 'cancelled'];
  if (closedStatuses.includes(String(ride.status || '').toLowerCase())) {
    return res.status(409).json({ error: 'Ce trajet ne peut plus etre modifie' });
  }

  const activeBookingRes = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(rideId)}&status=not.in.(annule_passager,annule_chauffeur,complete,completed,termine,cancelled,refunded,rembourse)&select=id,status,seats_reserved&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  );
  if (!activeBookingRes.ok) {
    return res.status(503).json({ error: 'Verification des reservations impossible. Modification bloquee par securite.' });
  }
  const activeBookings = await activeBookingRes.json().catch(() => []);
  if (activeBookings.length) {
    return res.status(409).json({
      error: 'Modification bloquee : ce trajet a deja une reservation active. Annule le trajet proprement ou cree un nouveau trajet pour changer les conditions.',
  ...
  if (has('departure_time')) {
    const d = new Date(body.departure_time);
    if (!Number.isFinite(d.getTime())) return res.status(400).json({ error: 'departure_time invalide' });
    if (d.getTime() < Date.now() - 5 * 60 * 1000) return res.status(400).json({ error: 'Le depart doit etre dans le futur' });
    patch.departure_time = d.toISOString();
  }

  if (has('return_departure_time')) {
    if (body.return_departure_time) {
      const rd = new Date(body.return_departure_time);
      if (!Number.isFinite(rd.getTime())) return res.status(400).json({ error: 'return_departure_time invalide' });
      patch.return_departure_time = rd.toISOString();
    } else {
      patch.return_departure_time = null;
    }
  }

  if (has('available_seats')) {
    const seats = Math.floor(Number(body.available_seats));
    if (!Number.isFinite(seats) || seats < 1 || seats > 8) {
      return res.status(400).json({ error: 'available_seats doit etre entre 1 et 8' });
    }
    patch.available_seats = seats;
    patch.total_seats = seats;
  }
  ...
  if (has('cost_per_km')) {
    ...
    if (!Number.isFinite(cpk) || cpk <= 0) return res.status(400).json({ error: 'cost_per_km invalide' });
    patch.cost_per_km = Math.min(cpk, eRate.max, RIDE_MAX_COST_PER_KM);
  }

  if (has('energy_type')) {
    const energyKey = String(body.energy_type || '').toLowerCase();
    if (['electrique','hybride','essence','diesel'].includes(energyKey)) patch.energy_type = energyKey;
  }
  if (has('start_city')) patch.start_city = String(body.start_city || '').trim().slice(0, 100);
  if (has('end_city')) patch.end_city = String(body.end_city || '').trim().slice(0, 100);
  if (has('start_sector')) patch.start_sector = body.start_sector ? String(body.start_sector).trim().slice(0, 120) : null;
  if (has('end_sector')) patch.end_sector = body.end_sector ? String(body.end_sector).trim().slice(0, 120) : null;
  if (has('start_meeting_point') || has('pickup_safe_label')) {
    const mp = body.start_meeting_point || body.pickup_safe_label;
    patch.start_meeting_point = mp ? String(mp).trim().slice(0, 240) : null;
  }
  if (has('end_meeting_point') || has('dropoff_safe_label')) {
    const mp = body.end_meeting_point || body.dropoff_safe_label;
    patch.end_meeting_point = mp ? String(mp).trim().slice(0, 240) : null;
  }
  if (has('vehicle_type')) patch.vehicle_type = String(body.vehicle_type || 'berline').slice(0, 30);
  if (has('trunk_size')) patch.trunk_size = ['petit','moyen','grand'].includes(body.trunk_size) ? body.trunk_size : 'moyen';
  if (has('accepts_pets')) patch.accepts_pets = toBool(body.accepts_pets);
  if (has('accepts_large_luggage')) patch.accepts_large_luggage = toBool(body.accepts_large_luggage);
  if (has('accepts_extra_stops')) patch.accepts_extra_stops = toBool(body.accepts_extra_stops);
  if (has('accepts_packages')) patch.accepts_packages = toBool(body.accepts_packages);
  if (has('ac_available')) patch.ac_available = toBool(body.ac_available);
  if (has('women_only')) patch.women_only = toBool(body.women_only);
  if (has('child_seat_available')) patch.child_seat_available = toBool(body.child_seat_available);
  if (has('accessible')) patch.accessible = toBool(body.accessible);
  if (has('is_recurring')) patch.is_recurring = toBool(body.is_recurring);
  if (has('is_return_trip')) patch.is_return_trip = toBool(body.is_return_trip);
  if (has('recurrence_days')) patch.recurrence_days = Array.isArray(body.recurrence_days) ? body.recurrence_days.slice(0, 7) : null;
  if (has('package_max_kg')) patch.package_max_kg = body.package_max_kg ? Math.min(Math.max(Number(body.package_max_kg) || 0, 0), 50) : 10;
  if (has('package_max_dim_cm')) patch.package_max_dim_cm = body.package_max_dim_cm ? Math.min(Math.max(Number(body.package_max_dim_cm) || 0, 0), 200) : 60;
  if (has('flexibility_minutes')) patch.flexibility_minutes = Math.max(0, Math.min(Number(body.flexibility_minutes) || 0, 180));
  if (has('total_distance_km')) patch.total_distance_km = Math.max(1, Math.min(Number(body.total_distance_km) || 1, 2000));
  if (has('personal_rules')) patch.personal_rules = body.personal_rules ? String(body.personal_rules).slice(0, 500) : null;
  if (has('smoking_policy')) patch.smoking_policy = ['non_fumeur','fumeur','exterieur'].includes(body.smoking_policy) ? body.smoking_policy : 'non_fumeur';
  if (has('music_policy')) patch.music_policy = ['silence','selon_humeur','musique'].includes(body.music_policy) ? body.music_policy : 'selon_humeur';
  if (has('chat_policy')) patch.chat_policy = ['silencieux','selon_humeur','bavard'].includes(body.chat_policy) ? body.chat_policy : 'selon_humeur';
  ...
  for (let tries = 0; tries < 4; tries += 1) {
    updateRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}`, {
      method: 'PATCH',
      headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
    ...
    const errStr = String(data?.message || data?.error?.message || data?.hint || data?.details || JSON.stringify(data || {}));
    const missing =
      errStr.match(/could not find\s+the\s+["'`\\]+(\w+)["'`\\]+\s+column/i)?.[1] ||
      errStr.match(/column\s+["'`\\]+(\w+)["'`\\]+\s+(does not exist|not found|of relation)/i)?.[1] ||
      errStr.match(/column\s+(\w+)\s+(does not exist|not found|of relation)/i)?.[1] ||
      errStr.match(/could not find\s+["'`\\]+(\w+)["'`\\]+/i)?.[1];
    if (!missing || updatePayload[missing] === undefined) break;
    delete updatePayload[missing];
  }
  if (!updateRes.ok) {
    return res.status(400).json({ error: 'Modification trajet impossible', details: data });
  }

  const updated = Array.isArray(data) ? data[0] : data;
  return res.status(200).json({ success: true, ride: updated, blocked_active_bookings: false });
}
...
async function rideDetail(req, res, ctx, body) {
  const url    = new URL(req.url || '/', 'https://porteaporte.site');
  const rideId = safeRestId(body.ride_id || url.searchParams.get('ride_id'));
  if (!rideId) return res.status(400).json({ error: 'ride_id requis ou invalide' });

  const r = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=*`, {
    headers: sbHeaders(ctx.sbKey),
  });
  const rows = r.ok ? await r.json() : [];
  ...
  const stopsRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_stops?ride_id=eq.${encodeURIComponent(rideId)}&order=stop_order.asc`, {
    headers: sbHeaders(ctx.sbKey),
  });
  ...
  if (safePointIds.length) {
    try {
      const idsFilter = safePointIds.map(id => `id.eq.${encodeURIComponent(id)}`).join(',');
...
async function rideBook(req, res, ctx, body) {
  ...
  const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(ride_id)}&select=id,driver_id,start_city,end_city,departure_time,available_seats,total_seats,status,cost_per_km,total_distance_km,accepts_pets,accepts_large_luggage,commission_free`, {
    headers: sbHeaders(ctx.sbKey),
  });
  ...
  if (has_pet) {
    const petRes = await fetch(
      `${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ctx.session.id)}&select=pet_name,pet_species,pet_photo_url,pet_photo_status`,
      { headers: sbHeaders(ctx.sbKey) }
    );
    if (!petRes.ok) {
      return res.status(400).json({
        error: 'Carte animal requise avant de reserver avec un animal. Complete ton profil animal et fais approuver la photo.'
      });
    }
    const petRows = await petRes.json().catch(() => []);
    const pet = petRows[0] || {};
    if (!pet.pet_name || !pet.pet_species) {
      return res.status(400).json({
        error: 'Complete la carte animal avant de reserver: nom et type animal requis.'
      });
    }
    if (pet.pet_photo_status !== 'approved' || !pet.pet_photo_url) {
      const pending = pet.pet_photo_status === 'pending';
      return res.status(400).json({
        error: pending
          ? 'La photo de ton animal est en attente d approbation. Tu pourras reserver avec animal quand elle sera approuvee.'
          : 'Ajoute une photo claire de ton animal et attends son approbation avant de reserver avec animal.'
      });
    }
  }
  ...
  // ── Vérification PRÉ-INSERT : éviter l'erreur UNIQUE (ride_id, passenger_id) ──
  let reusedCancelledBooking = false;
  const preCheckRes = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(ride_id)}&passenger_id=eq.${encodeURIComponent(ctx.session.id)}&select=id,status&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  );
  const preCheck = preCheckRes.ok ? await preCheckRes.json().catch(() => []) : [];
  const preExisting = preCheck[0] || null;

  let bRes, bData;

  if (preExisting) {
    const preStatus = String(preExisting.status || '').toLowerCase();
    const isReusable = preStatus.startsWith('annule') || ['cancelled','canceled','refunded','rembourse'].includes(preStatus);
    if (!isReusable) {
      // Réservation active existante — vraiment prise
      return res.status(409).json({ error: 'Tu as déjà une réservation active pour ce trajet.', booking_id: preExisting.id });
    }
    // Réservation annulée — on la réutilise via PATCH
    reusedCancelledBooking = true;
    bRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(preExisting.id)}`, {
      method: 'PATCH',
      headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
      body: JSON.stringify({
        ...booking,
    ...
    // Pas de réservation existante — INSERT normal
    bRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings`, {
      method: 'POST',
      headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
  ...
  if (!bRes.ok) {
    ...
    if (code === '23505' || /duplicate key/i.test(msg)) {
      ...
      if (/ride_id.*passenger_id/i.test(bData?.details || '')) {
        ...
        const existRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(ride_id)}&passenger_id=eq.${encodeURIComponent(ctx.session.id)}&select=id,status,created_at,seats_reserved`, {
          headers: sbHeaders(ctx.sbKey)
        });
        const exist = existRes.ok ? (await existRes.json())[0] : null;
        const existingStatus = String(exist?.status || '').toLowerCase();
        const reusable = existingStatus.startsWith('annule')
          || ['cancelled', 'canceled', 'refunded', 'rembourse'].includes(existingStatus);
        if (exist?.id && reusable) {
          bRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(exist.id)}`, {
            method: 'PATCH',
            headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
            body: JSON.stringify({
              ...booking,
          ...
          bData = await bRes.json().catch(() => ({}));
          if (!bRes.ok) {
            return res.status(400).json({ error: 'Reservation annulee impossible a reactiver', details: bData });
          }
          reusedCancelledBooking = true;
        } else {
        return res.status(409).json({
    ...
    if (!bRes.ok && (code === '23503' || /foreign key/i.test(msg))) {
      return res.status(400).json({ error: 'Trajet ou utilisateur introuvable. Recharge la page.' });
    }
    if (bRes.ok) {
      // Reservation annulee reactivee: continuer le flux normal.
    } else
  ...
  if (reusedCancelledBooking) {
    // On réutilise une réservation annulée — les places ont déjà été décrémentées
    // lors de la réservation originale. On ne décrémente pas à nouveau.
    // On restaure juste le statut du trajet si nécessaire.
    const currentSeats = Number(ride.available_seats || 0);
    if (currentSeats <= 0 && ride.status === 'complet') {
      // Trajet marqué complet mais on réutilise une résa annulée — laisser tel quel
    }
    seatPatchRows = [{ id: ride_id }]; // Simuler succès
  } else {
    ...
    seatPatchRows = seatPatchRes.ok ? await seatPatchRes.json().catch(() => []) : [];
  ...
  try {
    await sendRideBookingToDriverEmail(ctx, saved);
  } catch (e) {
    console.warn('[rideBook] email chauffeur echec:', e.message);
  }

  const driverRes = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ride.driver_id)}&select=prenom,telephone,email,cov_rating_avg&limit=1`, {
    headers: sbHeaders(ctx.sbKey)
  }).catch(() => ({ ok: false }));
  const driverRows = driverRes.ok ? await driverRes.json().catch(() => []) : [];
  const driver = driverRows[0] || {};
  const contactAllowed = ['confirme', 'driver_completed', 'paye']
    .includes(String(saved.status || '').toLowerCase());

  return res.status(200).json({
    success: true,
    booking: saved,
    ride: {
      start_city: ride.start_city || null,
      end_city: ride.end_city || null,
      departure_time: ride.departure_time || null,
      pickup_city: saved.pickup_city || ride.pickup_city || null,
      pickup_sector: saved.pickup_sector || ride.pickup_sector || null,
      dropoff_city: saved.dropoff_city || ride.dropoff_city || null,
      dropoff_sector: saved.dropoff_sector || ride.dropoff_sector || null,
    },
    driver: {
      prenom: driver.prenom || null,
      cov_rating_avg: driver.cov_rating_avg ?? null,
      ...(contactAllowed ? {
        has_phone: !!driver.telephone,
        has_email: !!driver.email,
      } : {}),
    },
    price_breakdown: price,
    reused_cancelled_booking: reusedCancelledBooking
  });
}

async function rideDriverContact(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const bookingId = safeRestId(body?.booking_id);
  if (!bookingId) return res.status(400).json({ error: 'booking_id requis ou invalide' });

  const bookingRes = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(bookingId)}&passenger_id=eq.${encodeURIComponent(ctx.session.id)}&select=id,ride_id,passenger_id,status&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  ...
  const allowed = ['confirme', 'driver_completed', 'paye'];
  if (!allowed.includes(String(booking.status || '').toLowerCase())) {
    return res.status(403).json({ error: 'Coordonnees disponibles seulement apres confirmation' });
  }

  const rideRes = await fetch(
    `${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(booking.ride_id)}&select=id,driver_id&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const rideRows = rideRes.ok ? await rideRes.json().catch(() => []) : [];
  const ride = rideRows[0] || null;
  if (!ride?.driver_id) return res.status(404).json({ error: 'Conducteur introuvable' });

  const driverRes = await fetch(
    `${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ride.driver_id)}&select=telephone,email&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const driverRows = driverRes.ok ? await driverRes.json().catch(() => []) : [];
  const driver = driverRows[0] || {};
...
async function rideSendMeetupMessage(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const bookingId = safeRestId(body?.booking_id);
  const message = String(body?.message || '').trim();
  if (!bookingId) return res.status(400).json({ error: 'booking_id requis ou invalide' });
  if (!message) return res.status(400).json({ error: 'message requis' });
  if (message.length > 300) return res.status(400).json({ error: 'Message trop long (maximum 300 caracteres)' });

  const bookingRes = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(bookingId)}&select=id,ride_id,passenger_id,status&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  );
  const bookingRows = bookingRes.ok ? await bookingRes.json().catch(() => []) : [];
  const booking = bookingRows[0] || null;
  if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });
  if (String(booking.status || '').toLowerCase() !== 'confirme') {
    return res.status(409).json({ error: 'Le message de point de rencontre est disponible seulement pour les reservations confirmees' });
  }

  const rideRes = await fetch(
    `${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(booking.ride_id)}&select=id,driver_id,start_city,start_sector,end_city,end_sector,departure_time&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  );
  const rideRows = rideRes.ok ? await rideRes.json().catch(() => []) : [];
  const ride = rideRows[0] || null;
  if (!ride) return res.status(404).json({ error: 'Trajet introuvable' });
  if (ride.driver_id !== ctx.session.id && !roleIn(ctx.profile, ['admin'])) {
    return res.status(403).json({ error: 'Seul le conducteur du trajet peut envoyer ce message' });
  }

  const patchRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
    method: 'PATCH',
    headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
    body: JSON.stringify({ meetup_message: message, meetup_message_at: new Date().toISOString() }),
  });
  const patched = await patchRes.json().catch(() => ({}));
  if (!patchRes.ok) {
    return res.status(400).json({
      error: 'Sauvegarde du message impossible. Verifie la migration SQL ride_bookings.meetup_message.',
      details: patched,
    });
  }
  ...
  try {
    const [paxRes, drvRes] = await Promise.all([
      fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(booking.passenger_id)}&select=email,prenom&limit=1`, { headers: sbHeaders(ctx.sbKey) }),
      fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(ride.driver_id)}&select=prenom&limit=1`, { headers: sbHeaders(ctx.sbKey) }),
    ]);
    const passenger = paxRes.ok ? (await paxRes.json().catch(() => []))[0] : null;
    const driver = drvRes.ok ? (await drvRes.json().catch(() => []))[0] : null;
    await sendRideMeetupPassengerEmail(ctx, { booking, ride, passenger, driver, message });
  } catch (e) {
    console.warn('[rideSendMeetupMessage] email passager echec:', e.message);
  }
...
function normalizeSafetyCode(code) {
  const value = String(code || '').replace(/\D/g, '').trim();
  return /^[0-9]{4,8}$/.test(value) ? value : null;
}

async function hashSafetyCode(code) {
  return bcrypt.hash(code, 10);
}

async function compareSafetyCode(code, hash) {
  if (!code || !hash) return false;
  return bcrypt.compare(code, hash);
}

async function fetchRideBookingSafety(ctx, bookingId) {
  const safeId = safeRestId(bookingId);
  if (!safeId) return null;
  const r = await fetch(
    `${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(safeId)}&select=id,ride_id,passenger_id,status,total_passenger,driver_amount,platform_fee,stripe_payment_intent,payment_status,paid_at,updated_at,safety_code_hash,safety_code_custom,safety_alert_triggered,safety_code_failed_attempts,alert_sent_at,last_gps_lat,last_gps_lng,last_gps_at&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const rows = r.ok ? await r.json().catch(() => []) : [];
  ...
  const rideId = safeRestId(booking.ride_id);
  if (rideId) {
    const rr = await fetch(
      `${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=id,driver_id,departure_time&limit=1`,
      { headers: sbHeaders(ctx.sbKey) }
    ).catch(() => ({ ok: false }));
    const rideRows = rr.ok ? await rr.json().catch(() => []) : [];
    booking.ride = rideRows[0] || null;
  }
  return booking;
}

async function fetchProfileSafetyHash(ctx, userId) {
  const safeId = safeRestId(userId);
  if (!safeId) return null;
  const r = await fetch(
    `${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(safeId)}&select=id,ride_safety_code_hash&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const rows = r.ok ? await r.json().catch(() => []) : [];
  return rows[0]?.ride_safety_code_hash || null;
}
...
function canReadRideBooking(ctx, booking) {
  const uid = ctx.session?.id;
  return !!(ctx.profile?.role === 'admin'
    || booking.passenger_id === uid
    || booking.ride?.driver_id === uid);
}

async function completeRideBookingWithCapture(req, res, ctx, booking, actor) {
  let captured = null;
  const fakeRes = {
    status: (code) => ({
  ...
  await rideCaptureEligible(req, fakeRes, ctx, { booking_id: booking.id, force: true, grace_hours: 0 });
  const result = captured?.data || {};
  const errors = result.errors || [];
  if (errors.length) {
    return res.status(502).json({ error: errors[0].error || 'Capture impossible', capture: result });
  }
  if (!(result.captured || []).some((row) => row && row.booking_id === booking.id)) {
    const skipped = (result.skipped || []).find((row) => row && row.booking_id === booking.id);
    return res.status(409).json({
      error: skipped?.reason || 'Capture non effectuee',
      capture: result
    });
  }
  ...
  const patch = {
    ...
    safety_code_verified_at: actor === 'auto_timeout' ? null : now,
    safety_alert_triggered: false,
  };
  if (actor === 'passenger') patch.passenger_confirmed_at = now;

  await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify(patch)
  }).catch(() => null);
...
function normalizeEmergencyContact(raw) {
  const priority = Math.trunc(toNumber(raw?.priority, 0));
  if (priority < 1 || priority > 3) return null;
  const isPorteaporte = raw?.is_porteaporte === true;
  const phone = String(raw?.phone || '').trim().slice(0, 40);
  const email = String(raw?.email || '').trim().toLowerCase().slice(0, 160);
  const name = String(raw?.name || (isPorteaporte ? 'PorteaPorte' : '')).trim().slice(0, 120);
  if (!name) return null;
  if (!isPorteaporte && !phone && !email) return null;
  return {
    priority,
    name,
    phone: phone || null,
    email: email || null,
    is_porteaporte: isPorteaporte,
  };
}

async function fetchEmergencyContacts(ctx, userId) {
  const safeId = safeRestId(userId);
  if (!safeId) return [];
  const r = await fetch(
    `${ctx.sbUrl}/rest/v1/emergency_contacts?user_id=eq.${encodeURIComponent(safeId)}&select=id,priority,name,phone,email,is_porteaporte,updated_at&order=priority.asc`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  return r.ok ? await r.json().catch(() => []) : [];
}

async function emergencyContactsGet(req, res, ctx) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const contacts = await fetchEmergencyContacts(ctx, ctx.session.id);
  return res.status(200).json({ contacts });
}

async function emergencyContactsSet(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const list = Array.isArray(body?.contacts) ? body.contacts : [];
  const byPriority = new Map();
  for (const item of list.slice(0, 3)) {
    const contact = normalizeEmergencyContact(item);
    if (contact) byPriority.set(contact.priority, contact);
  }
  const contacts = Array.from(byPriority.values()).sort((a, b) => a.priority - b.priority);
  if (!contacts.length) {
    return res.status(400).json({ error: 'Configure au moins un contact: telephone, email ou PorteaPorte.' });
  }

  const uid = safeRestId(ctx.session.id);
  const now = new Date().toISOString();
  await fetch(`${ctx.sbUrl}/rest/v1/emergency_contacts?user_id=eq.${encodeURIComponent(uid)}`, {
    method: 'DELETE',
  ...
  const r = await fetch(`${ctx.sbUrl}/rest/v1/emergency_contacts`, {
    method: 'POST',
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify(rows)
  });
  if (!r.ok) return res.status(502).json({ error: 'Impossible de sauvegarder les contacts urgence' });
  const saved = await r.json().catch(() => rows);
  return res.status(200).json({ success: true, contacts: saved });
}

function validRideLatLng(lat, lng) {
  return lat !== null && lng !== null && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;
}

async function rideGpsUpdate(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const rideId = safeRestId(body?.ride_id);
  const bookingId = safeRestId(body?.booking_id);
  if (!rideId || !bookingId) return res.status(400).json({ error: 'ride_id et booking_id requis' });
  const lat = toNumber(body?.lat);
  const lng = toNumber(body?.lng);
  if (!validRideLatLng(lat, lng)) return res.status(400).json({ error: 'Coordonnees GPS invalides' });

  const booking = await fetchRideBookingSafety(ctx, bookingId);
  if (!booking || booking.ride_id !== rideId) return res.status(404).json({ error: 'Reservation introuvable' });
  if (booking.ride?.driver_id !== ctx.session.id && ctx.profile?.role !== 'admin') {
    return res.status(403).json({ error: 'GPS reserve au chauffeur du trajet' });
  }
  ...
  const update = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(bookingId)}`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify(patch)
  });
  if (!update.ok) return res.status(502).json({ error: 'GPS covoiturage impossible' });

  let trail_saved = false;
  if (booking.safety_alert_triggered) {
    const trail = await fetch(`${ctx.sbUrl}/rest/v1/ride_gps_trail`, {
      method: 'POST',
      headers: sbHeaders(ctx.sbKey),
      body: JSON.stringify({ booking_id: bookingId, ride_id: rideId, lat, lng, recorded_at: now })
    });
    trail_saved = trail.ok;
  }

  return res.status(200).json({ success: true, alert_active: !!booking.safety_alert_triggered, trail_saved });
}

async function rideSafetyAlert(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  const internal = body?.internal === true;
  if (!internal && ctx.profile?.role !== 'admin') return res.status(403).json({ error: 'Alerte reservee a l equipe PorteaPorte' });

  const booking = await fetchRideBookingSafety(ctx, body?.booking_id);
  if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });

  const passengerId = safeRestId(booking.passenger_id);
  const driverId = safeRestId(booking.ride?.driver_id);
  const profileIds = [passengerId, driverId].filter(Boolean);
  let profiles = [];
  if (profileIds.length) {
    const p = await fetch(
      `${ctx.sbUrl}/rest/v1/profiles?id=in.(${profileIds.map(encodeURIComponent).join(',')})&select=id,prenom,nom,email,telephone,phone`,
      { headers: sbHeaders(ctx.sbKey) }
    ).catch(() => ({ ok: false }));
    profiles = p.ok ? await p.json().catch(() => []) : [];
  }
  const passenger = profiles.find((p) => p.id === passengerId) || {};
  const driver = profiles.find((p) => p.id === driverId) || {};
  let contacts = await fetchEmergencyContacts(ctx, booking.passenger_id);
  if (!contacts.length) {
    contacts = [{ priority: 1, name: 'PorteaPorte', phone: null, email: null, is_porteaporte: true }];
  }

  const ride = booking.ride || {};
  const lastGps = booking.last_gps_lat != null && booking.last_gps_lng != null
    ? `${booking.last_gps_lat},${booking.last_gps_lng}`
    : 'GPS non disponible';
  const message = [
    'Alerte securite covoiturage: 3 codes invalides.',
    `Booking: ${booking.id}`,
    `Trajet: ${ride.id || booking.ride_id}`,
    `Passager: ${passenger.prenom || passenger.email || booking.passenger_id}`,
    `Chauffeur: ${driver.prenom || driver.email || ride.driver_id || 'inconnu'}`,
    `Dernier GPS: ${lastGps}`
  ].join('\n');
  ...
  await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
    ...
    body: JSON.stringify({
      ...
      alert_sent_at: booking.alert_sent_at || now,
      safety_code_failed_attempts: Math.max(3, Number(booking.safety_code_failed_attempts || 0)),
      updated_at: now,
    })
  }).catch(() => null);

  alertAdmin('Alerte securite covoiturage', message, {
    severity: 'critical',
    details: {
      booking_id: booking.id,
      ...
      driver_id: ride.driver_id || null,
      last_gps: lastGps,
      contacts: contacts.map((c) => `${c.priority}. ${c.name}${c.is_porteaporte ? ' (PorteaPorte)' : ''}`).join(' | ')
    },
    cta_url: 'https://porteaporte.site/admin/ride-payments.html',
    cta_label: 'Ouvrir admin covoiturage'
  });
...
async function rideSafetyCodeStatus(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  body = body || {};
  const profileHash = await fetchProfileSafetyHash(ctx, ctx.session.id);
  const out = { has_code: !!profileHash, has_profile_code: !!profileHash };

  if (body.booking_id) {
    const booking = await fetchRideBookingSafety(ctx, body.booking_id);
    if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });
    if (!canReadRideBooking(ctx, booking)) return res.status(403).json({ error: 'Non autorise' });
    out.booking_id = booking.id;
    out.has_booking_code = !!booking.safety_code_hash;
    out.safety_code_custom = !!booking.safety_code_custom;
  }

  return res.status(200).json(out);
}

async function rideSafetyCodeSet(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  body = body || {};
  const code = normalizeSafetyCode(body.code);
  if (!code) return res.status(400).json({ error: 'Code invalide. Utilise 4 a 8 chiffres.' });
  const hash = await hashSafetyCode(code);
  const secretQuestion = String(body.safety_secret_question || body.secret_question || '').trim();
  const secretAnswer = normalizeSecretAnswer(body.safety_secret_answer || body.secret_answer);
  const now = new Date().toISOString();
  ...
  if (body.booking_id) {
    ...
    if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });
    if (booking.passenger_id !== ctx.session.id && ctx.profile?.role !== 'admin') {
      return res.status(403).json({ error: 'Seul le passager peut definir le code de cette reservation' });
    }
    const r = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
      method: 'PATCH',
      headers: sbHeaders(ctx.sbKey),
      body: JSON.stringify({
        safety_code_hash: hash,
        safety_code_custom: true,
        safety_code_set_at: now,
        safety_alert_triggered: false,
        safety_code_failed_attempts: 0,
      })
    });
    if (!r.ok) return res.status(502).json({ error: 'Impossible de sauvegarder le code reservation' });
    return res.status(200).json({ success: true, booking_id: booking.id, has_booking_code: true });
  }

  const uid = safeRestId(ctx.session.id);
  const profilePatch = { ride_safety_code_hash: hash, ride_safety_code_set_at: now };
  if (secretQuestion && secretAnswer) {
    profilePatch.safety_secret_question = secretQuestion.slice(0, 160);
    profilePatch.safety_secret_answer_hash = await hashSafetyCode(secretAnswer);
  }

  const r = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(uid)}`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify(profilePatch)
  });
  if (!r.ok) return res.status(502).json({ error: 'Impossible de sauvegarder le code securite' });
  return res.status(200).json({ success: true, has_profile_code: true });
}

async function rideSafetyCodeReset(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  body = body || {};
  const code = normalizeSafetyCode(body.code || body.new_code);
  const answer = normalizeSecretAnswer(body.safety_secret_answer || body.secret_answer);
  if (!code) return res.status(400).json({ error: 'Nouveau code invalide. Utilise 4 a 8 chiffres.' });
  if (!answer) return res.status(400).json({ error: 'Reponse secrete requise' });

  const uid = safeRestId(ctx.session.id);
  const profileRes = await fetch(
    `${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(uid)}&select=id,safety_secret_question,safety_secret_answer_hash&limit=1`,
    { headers: sbHeaders(ctx.sbKey) }
  ).catch(() => ({ ok: false }));
  const profileRows = profileRes.ok ? await profileRes.json().catch(() => []) : [];
  const profile = profileRows[0] || null;
  if (!profile?.safety_secret_answer_hash) {
    return res.status(409).json({ error: 'Aucune question secrete configuree' });
  }
  if (!await compareSafetyCode(answer, profile.safety_secret_answer_hash)) {
    return res.status(403).json({ error: 'Reponse secrete invalide' });
  }
  ...
  const r = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(uid)}`, {
    ...
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify({
      ride_safety_code_hash: await hashSafetyCode(code),
      ride_safety_code_set_at: new Date().toISOString(),
    })
  });
  if (!r.ok) return res.status(502).json({ error: 'Impossible de reinitialiser le code securite' });
  return res.status(200).json({ success: true, has_code: true });
}

async function rideDriverComplete(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  body = body || {};
  const booking = await fetchRideBookingSafety(ctx, body.booking_id);
  if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });
  if (booking.ride?.driver_id !== ctx.session.id && ctx.profile?.role !== 'admin') {
    return res.status(403).json({ error: 'Conducteur requis' });
  }

  const code = normalizeSafetyCode(body.code);
  if (code) {
    return rideComplete(req, res, ctx, { booking_id: booking.id, code, actor: 'driver' });
  }
  ...
  if (!r.ok) return res.status(502).json({ error: 'Impossible de marquer le trajet termine' });
  try {
    await sendRideDriverCompletedPassengerEmail(ctx, booking);
  } catch (e) {
    console.warn('[rideDriverComplete] email passager echec:', e.message);
  }
  return res.status(200).json({
    success: true,
    booking_id: booking.id,
    status: 'driver_completed',
    awaiting_passenger_confirmation: true,
  });
}
...
async function rideComplete(req, res, ctx, body) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  body = body || {};
  const actor = String(body.actor || 'passenger');
  if (actor === 'driver' && body.ride_id && !body.booking_id) {
    const rideId = safeRestId(body.ride_id);
    if (!rideId) return res.status(400).json({ error: 'ride_id invalide' });
    const rideRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=id,driver_id,status,departure_time&limit=1`, {
      headers: sbHeaders(ctx.sbKey)
    });
    const ride = rideRes.ok ? (await rideRes.json().catch(() => []))[0] : null;
    if (!ride) return res.status(404).json({ error: 'Trajet introuvable' });
    ...
    if (ride.departure_time && new Date(ride.departure_time) > new Date()) {
      return res.status(400).json({ error: 'Impossible de terminer un trajet avant son heure de depart' });
    }

    const bRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(rideId)}&status=eq.confirme&select=id,ride_id,passenger_id,status`, {
      headers: sbHeaders(ctx.sbKey)
    });
    const bookings = bRes.ok ? await bRes.json().catch(() => []) : [];
    const now = new Date().toISOString();
    await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}`, {
      method: 'PATCH',
      headers: sbHeaders(ctx.sbKey, 'return=minimal'),
      body: JSON.stringify({ status: 'termine', updated_at: now })
    }).catch(() => null);

    const updated = [];
    for (const booking of bookings) {
      const patch = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
        method: 'PATCH',
      ...
      const saved = rows[0] || { ...booking, status: 'driver_completed' };
      updated.push(saved);
      try { await sendRideDriverCompletedPassengerEmail(ctx, saved); } catch (_) {}
    }
    ...
    return res.status(200).json({
      ...
      ride_id: rideId,
      status: 'driver_completed',
      bookings_notified: updated.length,
      bookings: updated.map((b) => b.id),
    });
  }
  const booking = await fetchRideBookingSafety(ctx, body.booking_id);
  if (!booking) return res.status(404).json({ error: 'Reservation introuvable' });
  ...
  const uid = ctx.session.id;
  const isAdminOrCron = ctx.profile?.role === 'admin' || uid === '__cron__';
  const isPassenger = booking.passenger_id === uid;
  const isDriver = booking.ride?.driver_id === uid;

  if (actor === 'auto_timeout') {
    if (!isAdminOrCron) return res.status(403).json({ error: 'Auto-validation reservee au cron' });
    if (booking.status !== 'driver_completed') return res.status(409).json({ error: 'Reservation pas en attente auto-validation' });
    if (booking.safety_alert_triggered) return res.status(409).json({ error: 'Alerte securite active' });
    return completeRideBookingWithCapture(req, res, ctx, booking, actor);
  }

  if (actor === 'driver' && !isDriver && !isAdminOrCron) return res.status(403).json({ error: 'Conducteur requis' });
  if (actor !== 'driver' && !isPassenger && !isAdminOrCron) return res.status(403).json({ error: 'Passager requis' });

  const code = normalizeSafetyCode(body.code);
  if (!code) return res.status(400).json({ error: 'Code requis' });
  const profileHash = await fetchProfileSafetyHash(ctx, booking.passenger_id);
  const hash = booking.safety_code_hash || profileHash;
  if (!hash) return res.status(409).json({ error: 'Aucun code de securite configure' });
  if (!await compareSafetyCode(code, hash)) {
    const attempts = Number(booking.safety_code_failed_attempts || 0) + 1;
    await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
      method: 'PATCH',
      headers: sbHeaders(ctx.sbKey),
      body: JSON.stringify({
        safety_code_failed_attempts: attempts,
        safety_alert_triggered: attempts >= 3 ? true : !!booking.safety_alert_triggered,
        updated_at: new Date().toISOString(),
      })
    }).catch(() => null);
    if (attempts >= 3) {
      await rideSafetyAlert(req, { status: () => ({ json: (data) => data }) }, ctx, {
        booking_id: booking.id,
        internal: true,
      }).catch(() => null);
    }
    return res.status(403).json({
      error: attempts >= 3 ? 'Code invalide. Alerte securite active.' : 'Code invalide',
  ...
  await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(booking.id)}`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey, 'return=minimal'),
    body: JSON.stringify({ safety_code_failed_attempts: 0 })
  }).catch(() => null);
...
async function transferToDriverConnect(ctx, driverId, amountCents, currency, sourceCharge, bookingId) {
  ...
  if (!acct || !acct.stripe_account_id) {
    return { status: 'manual_review', transferId: null, reason: 'connect_missing' };
  }
  let connectStatus = acct.status;
  if (connectStatus !== 'active') {
    const stripeAcct = await stripeGetConnectAccount(acct.stripe_account_id, ctx.stripeKey);
    if (stripeAcct) {
      connectStatus = stripeAcct.charges_enabled && stripeAcct.payouts_enabled ? 'active'
                    : stripeAcct.details_submitted ? 'onboarding'
                    : 'pending';
      await fetch(`${ctx.sbUrl}/rest/v1/stripe_connect_accounts?user_id=eq.${encodeURIComponent(driverId)}`, {
        method: 'PATCH',
        headers: sbHeaders(ctx.sbKey, 'return=minimal'),
        body: JSON.stringify({
  ...
  if (connectStatus !== 'active') {
    return { status: 'manual_review', transferId: null, reason: 'connect_' + connectStatus };
  }
  const params = new URLSearchParams({
    amount: String(amountCents),
    currency: currency || 'cad',
    destination: acct.stripe_account_id,
    description: 'PorteaPorte covoit booking ' + String(bookingId).slice(0, 8),
  });
...
async function updateRideCancellationMetrics(ctx, { userId, actor, penalty = 0 }) {
  if (!userId || !['passenger', 'driver'].includes(actor)) return { updated: false };
  try {
    const isDriver = actor === 'driver';
    const historyUrl = isDriver
      ? `${ctx.sbUrl}/rest/v1/rides?driver_id=eq.${encodeURIComponent(userId)}&select=id,status,created_at&limit=1000`
      : `${ctx.sbUrl}/rest/v1/ride_bookings?passenger_id=eq.${encodeURIComponent(userId)}&select=id,status,created_at&limit=1000`;
    const historyRes = await fetch(historyUrl, { headers: sbHeaders(ctx.sbKey) });
    const history = historyRes.ok ? await historyRes.json().catch(() => []) : [];
    const cancelledStatuses = isDriver ? new Set(['annule', 'annule_chauffeur']) : new Set(['annule_passager']);
    const cancelled = history.filter((row) => cancelledStatuses.has(String(row.status || '').toLowerCase()));
    const rate = history.length ? Math.round((cancelled.length / history.length) * 10000) / 100 : 0;

    const profileRes = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}&select=id,score,score_fiabilite`, {
      headers: sbHeaders(ctx.sbKey),
    });
    const profile = profileRes.ok ? (await profileRes.json().catch(() => []))[0] : null;
    const patch = { taux_annulation: rate };
    if (isDriver && penalty > 0 && profile) {
      const globalScore = Number.isFinite(Number(profile.score)) ? Number(profile.score) : 100;
      const reliabilityBase = Number(profile.score_fiabilite) > 0 ? Number(profile.score_fiabilite) : globalScore;
      patch.score = Math.max(0, globalScore - penalty);
      patch.score_fiabilite = Math.max(0, reliabilityBase - penalty);
    }
    const patchRes = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}`, {
      method: 'PATCH', headers: sbHeaders(ctx.sbKey, 'return=minimal'), body: JSON.stringify(patch),
    });
    const ninetyDaysAgo = Date.now() - 90 * 86400000;
    const recentCancelled = cancelled.filter((row) => !row.created_at || new Date(row.created_at).getTime() >= ninetyDaysAgo).length;
    const reviewRequired = isDriver && (recentCancelled >= 3 || (history.length >= 5 && rate > 20));
    if (reviewRequired) {
      alertAdmin('Révision fiabilité covoiturage', `Un conducteur atteint le seuil de révision des annulations.`, {
        severity: 'warning',
        details: { 'Conducteur': userId, 'Annulations (90 jours)': recentCancelled, 'Taux': `${rate} %`, 'Pénalité appliquée': `${penalty} points` },
        cta_url: 'https://porteaporte.site/gestion-pp-8k2x.html',
      });
    }
    return { updated: patchRes.ok, rate, recentCancelled, reviewRequired, penalty };
  } catch (err) {
    console.warn('[rideCancel] statistiques annulation non mises a jour:', err.message);
...
async function rideCancel(req, res, ctx, body) {
  ...
  const bookingId = booking_id ? safeRestId(booking_id) : null;
  const rideId = ride_id ? safeRestId(ride_id) : null;
  if (booking_id && !bookingId) return res.status(400).json({ error: 'booking_id invalide' });
  if (ride_id && !rideId) return res.status(400).json({ error: 'ride_id invalide' });

  if (bookingId) {
    const bRes = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(bookingId)}&select=id,passenger_id,ride_id,seats_reserved,status,total_passenger,driver_amount,stripe_payment_intent,payment_currency`, {
      headers: sbHeaders(ctx.sbKey),
    });
    ...
    const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(b.ride_id)}&select=id,driver_id,departure_time,available_seats,start_city,end_city`, {
      headers: sbHeaders(ctx.sbKey),
    });
    ...
    const bookingPatch = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(bookingId)}`, {
      method: 'PATCH',
      headers: sbHeaders(ctx.sbKey),
      body: JSON.stringify({ status: 'annule_passager' }),
    });
    ...
    if (ride) {
      const seatPatch = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(b.ride_id)}`, {
        method: 'PATCH',
        headers: sbHeaders(ctx.sbKey),
        body: JSON.stringify({ available_seats: (ride.available_seats || 0) + (b.seats_reserved || 1), status: 'publie' }),
      }).catch(() => ({ ok: false }));
    ...
    const cancellationMetrics = await updateRideCancellationMetrics(ctx, {
      userId: b.passenger_id,
      actor: 'passenger',
      penalty: 0,
    });

    // L'annulation financière et la remise en disponibilité sont déjà finalisées.
    // Une panne de courriel ne doit donc jamais transformer une annulation réussie en erreur.
    let notificationSent = false;
    try {
      const profileIds = [b.passenger_id, ride?.driver_id].filter(Boolean);
      const profilesRes = profileIds.length
        ? await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=in.(${profileIds.map(encodeURIComponent).join(',')})&select=id,email,prenom`, { headers: sbHeaders(ctx.sbKey) })
        : null;
      const profiles = profilesRes?.ok ? await profilesRes.json().catch(() => []) : [];
      const passenger = profiles.find((p) => p.id === b.passenger_id);
      const driver = profiles.find((p) => p.id === ride?.driver_id);
      const notifyRes = await postRideNotifier('ride_booking_cancelled', {
        booking_id: b.id,
        cancelled_by: 'passenger',
        passenger_email: passenger?.email || ctx.session.email || '',
        driver_email: driver?.email || '',
        ville_depart: ride?.start_city || '',
        ville_arrivee: ride?.end_city || '',
        seats: b.seats_reserved || 1,
        total_price: totalCents / 100,
        refund: refundCents / 100,
        currency,
      });
      notificationSent = !!notifyRes.ok;
      if (!notifyRes.ok) console.warn('[rideCancel] notification annulation non envoyee:', notifyRes.status);
    } catch (err) {
      console.warn('[rideCancel] notification annulation impossible:', err.message);
    }
    ...
    return res.status(200).json({
      ...
      cancellation_rate: cancellationMetrics.rate,
    });
  }

  if (rideId) {
    const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=id,driver_id,status,start_city,end_city,departure_time`, {
      headers: sbHeaders(ctx.sbKey),
    });
    const rRows = rRes.ok ? await rRes.json() : [];
    if (!rRows.length) return res.status(404).json({ error: 'Trajet introuvable' });
    ...
    const bRes = await fetch(
      `${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(rideId)}&status=not.in.(annule_passager,annule_chauffeur,complete)&select=id,passenger_id,status,stripe_payment_intent,total_passenger,seats_reserved,payment_currency`,
      { headers: sbHeaders(ctx.sbKey) }
    );
    const bookings = bRes.ok ? await bRes.json() : [];

    if (body.preview_only === true) {
      const totalRefund = bookings.reduce((sum, booking) => sum + (Number(booking.total_passenger) || 0), 0);
      const hoursUntil = ride.departure_time
        ? (new Date(ride.departure_time).getTime() - Date.now()) / 3600000
        : null;
      return res.status(200).json({
        preview: true,
        cancelled_by: 'driver',
        bookings_affected: bookings.length,
        total_refund: Math.round(totalRefund * 100) / 100,
        refund_pct: 100,
        hours_until_departure: Number.isFinite(hoursUntil) ? Math.round(hoursUntil * 10) / 10 : null,
        reliability_impact: 'pending_policy_approval',
      });
    }
    ...
    const ridePatch = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}`, {
      method: 'PATCH',
      headers: sbHeaders(ctx.sbKey),
    ...
    for (const b of bookings) {
      ...
      const bookingPatch = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=eq.${encodeURIComponent(b.id)}`, {
        method: 'PATCH',
        headers: sbHeaders(ctx.sbKey),
    ...
    const hoursUntilDeparture = ride.departure_time
      ? (new Date(ride.departure_time).getTime() - Date.now()) / 3600000
      : Infinity;
    const reliabilityPenalty = bookings.length === 0 ? 0 : hoursUntilDeparture >= 24 ? 0 : hoursUntilDeparture >= 2 ? 5 : 10;
    const cancellationMetrics = await updateRideCancellationMetrics(ctx, {
    ...
    try {
      const profileIds = [...new Set([ride.driver_id, ...bookings.map((b) => b.passenger_id)].filter(Boolean))];
      const profilesRes = profileIds.length
        ? await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=in.(${profileIds.map(encodeURIComponent).join(',')})&select=id,email,prenom`, { headers: sbHeaders(ctx.sbKey) })
        : null;
      const profiles = profilesRes?.ok ? await profilesRes.json().catch(() => []) : [];
      const driver = profiles.find((p) => p.id === ride.driver_id);
      const jobs = bookings.map(async (booking, index) => {
        const passenger = profiles.find((p) => p.id === booking.passenger_id);
        const notifyRes = await postRideNotifier('ride_booking_cancelled', {
          booking_id: booking.id,
          cancelled_by: 'driver',
          passenger_email: passenger?.email || '',
          // Le conducteur reçoit une seule confirmation, avec le premier avis.
          driver_email: index === 0 ? (driver?.email || ctx.session.email || '') : '',
          ville_depart: ride.start_city || '',
          ville_arrivee: ride.end_city || '',
          seats: booking.seats_reserved || 1,
          total_price: Number(booking.total_passenger) || 0,
          refund: Number(booking.total_passenger) || 0,
          currency: booking.payment_currency || 'cad',
        });
        return notifyRes.ok;
      });
      const notified = await Promise.allSettled(jobs);
      notificationsSent = notified.filter((result) => result.status === 'fulfilled' && result.value).length;
      if (notificationsSent !== bookings.length) {
        console.warn('[rideCancel] certains avis annulation conducteur non envoyes:', bookings.length - notificationsSent);
      }
    } catch (err) {
      console.warn('[rideCancel] avis annulation conducteur impossibles:', err.message);
    }
    ...
    return res.status(200).json({
      ...
      cancellation_rate: cancellationMetrics.rate,
      admin_review_required: cancellationMetrics.reviewRequired === true,
    });
  }
...
async function ridePackageBook(req, res, ctx, body) {
  ...
  const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(ride_id)}&select=id,driver_id,status,accepts_packages,package_max_kg,package_max_dim_cm,total_distance_km,cost_per_km,commission_free`, {
    headers: sbHeaders(ctx.sbKey),
  });
  const rRows = rRes.ok ? await rRes.json() : [];
  if (!rRows.length) return res.status(404).json({ error: 'Trajet introuvable' });
  ...
  // Frais plateforme configurable par palier (source unique : impact_settings)
  const platformFeePerSeat = ridePlatformFeePerSeat(packageFee, settings);
  // Franchise « N premiers trajets sans commission » figée à la publication du trajet
...
async function covDashboard(req, res, ctx) {
  ...
  const [
    ...
    fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?passenger_id=eq.${uid}&order=created_at.desc&limit=50&select=*,ride:rides(driver_id,start_city,end_city,departure_time,driver:profiles!driver_id(prenom))`, { headers: sbHeaders(ctx.sbKey) }),
  ...
  const contactAllowed = new Set(['confirme', 'driver_completed', 'paye']);
  const myBookings = myBookingsRaw.map(b => ({
    ...b,
    driver_prenom: b.ride?.driver?.prenom || null,
    ride_departure_time: b.ride?.departure_time || null,
    driver_contact_available: contactAllowed.has(String(b.status || '').toLowerCase()),
    ride: undefined
  }));
...
async function covProgress(req, res, ctx, body) {
  ...
  if (event === 'ride_complete' && ride_id) {
    const grRes  = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(ride_id)}&select=driver_id,energy_type&limit=1`, { headers: sbHeaders(ctx.sbKey) });
...
async function rideCaptureEligible(req, res, ctx, body) {
  try {
    ...
    if (body.booking_id) {
      const bookingId = safeRestId(body.booking_id);
      if (!bookingId) return res.status(400).json({ error: 'booking_id invalide' });
      const bRes = await fetch(ctx.sbUrl + '/rest/v1/ride_bookings?id=eq.' + encodeURIComponent(bookingId) + '&select=*&limit=1', {
        headers: sbHeaders(ctx.sbKey)
      });
      ...
      if (!isAdmin && b.passenger_id !== ctx.session.id) {
        const bRideId = safeRestId(b.ride_id);
        if (!bRideId) return res.status(400).json({ error: 'ride_id invalide' });
        const rRes = await fetch(ctx.sbUrl + '/rest/v1/rides?id=eq.' + encodeURIComponent(bRideId) + '&select=driver_id&limit=1', {
          headers: sbHeaders(ctx.sbKey)
        });
    ...
    for (const booking of bookings) {
      try {
        ...
        // 0) Délai de grâce : on ne capture/paie QU'APRÈS le trajet (+ graceHours).
        const bookingRideId = safeRestId(booking.ride_id);
        if (!bookingRideId) {
          errors.push({ booking_id: booking.id, error: 'ride_id invalide' });
          continue;
        }
        const rideInfoRes = await fetch(
          ctx.sbUrl + '/rest/v1/rides?id=eq.' + encodeURIComponent(bookingRideId) + '&select=driver_id,departure_time&limit=1',
          { headers: sbHeaders(ctx.sbKey) }
        );
        const rideInfo = rideInfoRes.ok ? (await rideInfoRes.json())[0] : null;
        const depMs = rideInfo && rideInfo.departure_time ? new Date(rideInfo.departure_time).getTime() : null;
        if (depMs !== null && depMs > Date.now() - graceHours * 3600 * 1000 && body.force !== true) {
          skipped.push({ booking_id: booking.id, reason: 'trip_not_finished' });
          continue;
        }
        ...
        if (isPlaceholder) {
          const placeholderBookingId = safeRestId(booking.id);
          if (!placeholderBookingId) {
            errors.push({ booking_id: booking.id, error: 'booking_id invalide' });
            continue;
          }
          await fetch(ctx.sbUrl + '/rest/v1/ride_bookings?id=eq.' + encodeURIComponent(placeholderBookingId), {
            method: 'PATCH',
            headers: sbHeaders(ctx.sbKey),
            body: JSON.stringify({ status: 'paye', payment_status: 'succeeded', paid_at: new Date().toISOString() })
          });
          captured.push({ booking_id: booking.id, captured: true, transferred: false, amount: booking.driver_amount, transfer_id: null, transfer_status: 'skipped', transfer_reason: 'placeholder', note: 'placeholder' });
          continue;
        }
        ...
        if (driverAmountCents > 0 && pi && pi.latest_charge) {
          const ride = rideInfo;  // déjà chargé (driver_id + departure_time) au début de la boucle
          if (ride && ride.driver_id) {
            const driverId = safeRestId(ride.driver_id);
            if (!driverId) {
              errors.push({ booking_id: booking.id, error: 'driver_id invalide' });
              continue;
            }
            const acctRes = await fetch(ctx.sbUrl + '/rest/v1/stripe_connect_accounts?user_id=eq.' + encodeURIComponent(driverId) + '&select=stripe_account_id,status&limit=1', {
              headers: sbHeaders(ctx.sbKey)
            });
            const acct = acctRes.ok ? (await acctRes.json())[0] : null;
        ...
        } else if (driverAmountCents > 0 && pi && pi.latest_charge && rideInfo && rideInfo.driver_id) {
          const driverId = safeRestId(rideInfo.driver_id);
          if (driverId) {
            const transfer = await transferToDriverConnect(ctx, driverId, driverAmountCents, pi.currency || 'cad', pi.latest_charge, booking.id);
            transferId = transfer.transferId || null;
            transferStatus = transfer.status;
            transferReason = transfer.reason || null;
          } else {
            transferReason = 'driver_id_invalide';
          }
        } else {
          transferReason = driverAmountCents > 0 ? 'missing_source_charge' : 'driver_amount_zero';
        }

        const capturedBookingId = safeRestId(booking.id);
        if (!capturedBookingId) {
          errors.push({ booking_id: booking.id, error: 'booking_id invalide' });
          continue;
        }
        await fetch(ctx.sbUrl + '/rest/v1/ride_bookings?id=eq.' + encodeURIComponent(capturedBookingId), {
          method: 'PATCH',
          headers: sbHeaders(ctx.sbKey),
          body: JSON.stringify({ status: 'paye', payment_status: 'succeeded', paid_at: new Date().toISOString() })
        });
        await patchRideTransactionMetadata(ctx, pi.id, {
          transfer_id: transferId,
          transfer_status: transferStatus,
          transfer_reason: transferReason,
          driver_amount_cents: driverAmountCents,
        }, { statut: 'succeeded' });
        await insertRideAudit(ctx, booking, pi, transferId ? 'ride_driver_transfer_complete' : 'ride_driver_transfer_skipped', {
          transfer_id: transferId,
          transfer_status: transferStatus,
          transfer_reason: transferReason,
          driver_amount_cents: driverAmountCents,
...
// ──────────────────────────────────────────────────────────────────────────
async function adminCleanupPhantomBookings(req, res, ctx) {
  if (!ctx.session) return res.status(401).json({ error: 'Authentification requise' });
  if (ctx.profile?.role !== 'admin' && ctx.session.id !== '__cron__') {
    return res.status(403).json({ error: 'Admin requis' });
  }

  const cutoff = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();
  const url = `${ctx.sbUrl}/rest/v1/ride_bookings?stripe_payment_intent=is.null&status=in.(en_attente,confirme)&created_at=lt.${encodeURIComponent(cutoff)}&select=id`;
  const r = await fetch(url, { headers: sbHeaders(ctx.sbKey) }).catch(() => ({ ok: false }));
  if (!r.ok) return res.status(502).json({ error: 'Recherche reservations fantomes impossible' });
  const rows = await r.json().catch(() => []);
  const ids = safeIds(rows.map((row) => row.id));
  if (!ids.length) return res.status(200).json({ success: true, cleaned: 0, booking_ids: [] });

  const patch = await fetch(`${ctx.sbUrl}/rest/v1/ride_bookings?id=in.(${ids.map(encodeURIComponent).join(',')})`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey, 'return=representation'),
    body: JSON.stringify({ status: 'annule_passager', updated_at: new Date().toISOString() })
  }).catch(() => ({ ok: false }));
  if (!patch.ok) return res.status(502).json({ error: 'Nettoyage reservations fantomes impossible' });
  const cleanedRows = await patch.json().catch(() => []);
  return res.status(200).json({
    success: true,
    cleaned: Array.isArray(cleanedRows) ? cleanedRows.length : ids.length,
    booking_ids: ids
  });
}
...
async function _ogFetchRide(ctx, rideId) {
  ...
  const pricing = ',total_seats,commission_free';
  const extra = pricing + ',accepts_pets,accepts_large_luggage,accepts_packages,accepts_extra_stops,smoking_policy,women_only,ac_available,trunk_size';
  ...
  try {
    ...
    if (row === undefined) row = await tryFetch(base + pricing);
    if (row === undefined) row = await tryFetch(base);
    return row || null;
  } catch (_) { return null; }
}
function _ogRideFacts(ride, rideSettings) {
  const price = calcRidePrice({
    totalDistanceKm: ride.total_distance_km,
    passengerDistanceKm: ride.total_distance_km,
    costPerKm: ride.cost_per_km,
  ...
  const passengerTotal = Math.max(0, Number(price.totalPassenger) || 0);
  let dateStr = '';
  try {
    if (ride.departure_time) {
  ...
  return {
    ...
    priceLabel: `${passengerTotal.toFixed(2)} $ / passager`,
    dateStr,
    seats: Number(ride.available_seats) || 0,
    options: _ogRideOptions(ride),
  };
}
...
async function rideOgPage(req, res, ctx) {
  ...
  if (ride && ride.status === 'publie') {
    const rideSettings = await getRideSettings(ctx);
    const f = _ogRideFacts(ride, rideSettings);
    title = `${ride.start_city} → ${ride.end_city} · ${f.priceLabel}`;
...
function _ogBuildSvg(ride, rideSettings) {
  const facts = ride ? _ogRideFacts(ride, rideSettings) : null;
  ...
  const priceLabel = facts ? facts.priceLabel : 'Prix juste · frais réels';
...
async function rideOgImage(req, res, ctx) {
  ...
  const rideSettings = ride ? await getRideSettings(ctx) : null;
  const svg = _ogBuildSvg(ride, rideSettings);
  try {
...
module.exports = {
  RIDE_COST_PER_KM, RIDE_PLATFORM_FEE, RIDE_PLATFORM_FEE_HIGH, RIDE_FEE_THRESHOLD, RIDE_PLATFORM_FEE_MIN, RIDE_MAX_COST_PER_KM, RIDE_MIN_CHARGE,
  RIDE_FEE_LUGGAGE, RIDE_FEE_PET, RIDE_FEE_STOP, RIDE_FEE_PACKAGE_BASE, RIDE_FEE_PACKAGE_PER_KG,
  RIDE_FREE_TRIPS_DEFAULT,
  getRideSettings, calcPackageFee, groupBonusPct, ridePlatformFeePerSeat, calcRidePrice,
  rideDriverProfile, rideCreate, rideUpdate, rideSearch, rideDetail, rideBook, rideDriverContact, rideSendMeetupMessage, ridePaymentCreate, ridePaymentSync,
  emergencyContactsGet, emergencyContactsSet, rideGpsUpdate, rideSafetyAlert,
  rideSafetyCodeStatus, rideSafetyCodeSet, rideSafetyCodeReset, rideDriverComplete, rideComplete, rideCancel,
  rideCaptureEligible, adminCleanupPhantomBookings,
