...
async function getRideSettings(ctx) {
  try {
    const base = 'ride_platform_pct,ride_fee_luggage,ride_fee_pet,ride_fee_stop,ride_fee_package_base,ride_fee_package_per_kg';
    // Tente d'inclure ride_free_trips ; si la colonne n'existe pas encore, on retombe sur le select de base
    let r = await fetch(`${ctx.sbUrl}/rest/v1/impact_settings?id=eq.default&select=${base},ride_free_trips&limit=1`, { headers: sbHeaders(ctx.sbKey) });
    if (!r.ok) {
      r = await fetch(`${ctx.sbUrl}/rest/v1/impact_settings?id=eq.default&select=${base}&limit=1`, { headers: sbHeaders(ctx.sbKey) });
    }
...
async function ridePackageBook(req, res, ctx, body) {
  ...
  // Franchise « N premiers trajets sans commission » figée à la publication du trajet
  const platformFee  = ride.commission_free ? 0 : Math.round(RIDE_PLATFORM_FEE * 100) / 100;
