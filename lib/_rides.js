...
async function _ogFetchRide(ctx, rideId) {
  ...
  try {
    const r = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=start_city,end_city,departure_time,available_seats,total_distance_km,cost_per_km,status&limit=1`, {
      headers: sbHeaders(ctx.sbKey),
    });
    const rows = r.ok ? await r.json() : [];
    return rows[0] || null;
...
function _ogRideFacts(ride) {
  ...
  try {
    if (ride.departure_time) {
      dateStr = new Date(ride.departure_time).toLocaleString('fr-CA', {
        weekday: 'long', day: 'numeric', month: 'long',
        hour: '2-digit', minute: '2-digit', timeZone: 'America/Toronto',
      });
    }
  } catch (_) {}
  return { price, dateStr, seats: Number(ride.available_seats) || 0 };
}
...
function _ogBuildSvg(ride) {
  ...
  const priceW = Math.max(280, priceLabel.length * 19 + 60);
  ...
  <text x="80" y="108" font-family="Arial, Helvetica, sans-serif" font-size="44" font-weight="700" fill="#5dbfff">PorteàPorte</text>
  <text x="80" y="148" font-family="Arial, Helvetica, sans-serif" font-size="26" fill="#8A9BB0">Covoiturage entre particuliers</text>
  ...
  <text x="150" y="306" font-family="Arial, Helvetica, sans-serif" font-size="${citySize}" font-weight="800" fill="#FFFFFF">${_ogEscapeXml(clip(from))}</text>
  <text x="150" y="408" font-family="Arial, Helvetica, sans-serif" font-size="${citySize}" font-weight="800" fill="#FFFFFF">${_ogEscapeXml(clip(to))}</text>
  <text x="80" y="492" font-family="Arial, Helvetica, sans-serif" font-size="30" fill="#B8C4D6">${_ogEscapeXml(dateLine)}</text>
  <rect x="80" y="520" width="${priceW}" height="66" rx="33" fill="url(#acc)"/>
  <text x="${80 + priceW / 2}" y="563" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="800" fill="#0A0F1E" text-anchor="middle">${_ogEscapeXml(priceLabel)}</text>
  <text x="${80 + priceW + 34}" y="563" font-family="Arial, Helvetica, sans-serif" font-size="30" fill="#8A9BB0">${_ogEscapeXml(seatsLabel)}</text>
  <text x="1120" y="598" font-family="Arial, Helvetica, sans-serif" font-size="24" fill="#56657D" text-anchor="end">porteaporte.site</text>
</svg>`;
}
