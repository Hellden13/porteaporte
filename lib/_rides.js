...
async function rideCancel(req, res, ctx, body) {
  ...
  if (bookingId) {
    ...
    const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(b.ride_id)}&select=id,driver_id,departure_time,available_seats`, {
      headers: sbHeaders(ctx.sbKey),
    });
  ...
  if (rideId) {
    const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}&select=id,driver_id,status`, {
      headers: sbHeaders(ctx.sbKey),
    });
    const rRows = rRes.ok ? await rRes.json() : [];
    if (!rRows.length) return res.status(404).json({ error: 'Trajet introuvable' });
    ...
    const bRes = await fetch(
      `${ctx.sbUrl}/rest/v1/ride_bookings?ride_id=eq.${encodeURIComponent(rideId)}&status=not.in.(annule_passager,annule_chauffeur,complete)&select=id,passenger_id,status,stripe_payment_intent`,
      { headers: sbHeaders(ctx.sbKey) }
    );
