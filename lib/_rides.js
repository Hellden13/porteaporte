...
async function rideUpdate(req, res, ctx, body) {
  ...
  const updateRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(rideId)}`, {
    method: 'PATCH',
    headers: { ...sbHeaders(ctx.sbKey), Prefer: 'return=representation' },
    body: JSON.stringify(patch),
  });
...
module.exports = {
  ...
  rideDriverProfile, rideCreate, rideUpdate, rideSearch, rideDetail, rideBook, rideDriverContact, ridePaymentCreate, ridePaymentSync,
