...
async function rideBook(req, res, ctx, body) {
  ...
  const rRes = await fetch(`${ctx.sbUrl}/rest/v1/rides?id=eq.${encodeURIComponent(ride_id)}&select=id,driver_id,start_city,end_city,departure_time,pickup_city,pickup_sector,dropoff_city,dropoff_sector,available_seats,total_seats,status,cost_per_km,total_distance_km,accepts_pets,accepts_large_luggage,commission_free`, {
    headers: sbHeaders(ctx.sbKey),
  });
...
module.exports = {
  ...
  rideDriverProfile, rideCreate, rideUpdate, rideSearch, rideDetail, rideBook, ridePaymentCreate, ridePaymentSync,
