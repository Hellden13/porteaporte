...
async function rideSafetyCodeSet(req, res, ctx, body) {
  ...
  const r = await fetch(`${ctx.sbUrl}/rest/v1/profiles?id=eq.${encodeURIComponent(uid)}`, {
    method: 'PATCH',
    headers: sbHeaders(ctx.sbKey),
    body: JSON.stringify({ ride_safety_code_hash: hash, ride_safety_code_set_at: now })
  });
...
module.exports = {
  ...
  rideSafetyCodeStatus, rideSafetyCodeSet, rideDriverComplete, rideComplete, rideCancel,
