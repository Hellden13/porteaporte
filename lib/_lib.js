...
function roleIn(profile, roles) {
  ...
  if (profileHasCapabilityColumns(profile)) {
    // 'admin' ne peut pas être accordé via les capability columns (déjà géré ci-dessus)
    const nonAdminWanted = wanted.filter(r => r !== 'admin');
    if (!nonAdminWanted.length) return false; // seul 'admin' demandé et user n'est pas admin
