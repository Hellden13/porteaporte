...
async function getProfile(userId, sbUrl, sbKey) {
  let r = await fetch(`${sbUrl}/rest/v1/profiles?id=eq.${userId}&select=id,email,prenom,nom,role,suspendu,email_verified,verification_status,driver_status,ville,vehicule,trajet_principal,mode_livraison,transport_mode,transport_mode_actif,transport_modes_disponibles,route_origine,route_destination,route_deviation_km,route_date,route_heure_debut,route_heure_fin,est_livreur,est_expediteur,est_passager,est_conducteur`, {
    headers: sbHeaders(sbKey)
  });
  if (!r.ok) {
    r = await fetch(`${sbUrl}/rest/v1/profiles?id=eq.${userId}&select=id,email,prenom,nom,role,suspendu,email_verified,verification_status,driver_status,ville,vehicule,trajet_principal,mode_livraison,transport_mode,transport_mode_actif,transport_modes_disponibles,route_origine,route_destination,route_deviation_km,route_date,route_heure_debut,route_heure_fin`, {
      headers: sbHeaders(sbKey)
    });
  }
  if (!r.ok) {
...
const CAPABILITY_KEYS = ['est_livreur', 'est_expediteur', 'est_passager', 'est_conducteur'];

function profileHasCapabilityColumns(profile) {
  return Boolean(profile && CAPABILITY_KEYS.some((key) => Object.prototype.hasOwnProperty.call(profile, key)));
}

function capabilityFallbackFromRole(profile, cap) {
  const role = normalizeRole(profile?.role);
  if (cap === 'est_livreur') return role === 'livreur' || role === 'les deux';
  if (cap === 'est_expediteur') return role === 'expediteur' || role === 'les deux';
  if (cap === 'est_passager') return true;
  if (cap === 'est_conducteur') return true;
  return false;
}
...
function hasCapability(profile, cap) {
  ...
  if (!CAPABILITY_KEYS.includes(key)) return false;
  if (Object.prototype.hasOwnProperty.call(profile, key)) {
    if (profile[key] === true) return true;
    if (profile[key] === false) return false;
    return capabilityFallbackFromRole(profile, key); // null → fallback sur le rôle
  }
  return capabilityFallbackFromRole(profile, key);
}

function deriveLegacyRole(capabilities = {}, currentRole = '') {
  if (normalizeRole(currentRole) === 'admin') return 'admin';
  const estLivreur = capabilities.est_livreur === true;
  const estExpediteur = capabilities.est_expediteur === true;
  if (estLivreur && estExpediteur) return 'les deux';
  if (estLivreur) return 'livreur';
  if (estExpediteur) return 'expediteur';
  return 'expediteur';
}
...
function roleIn(profile, roles) {
  ...
  const wanted = roles.map((role) => normalizeRole(role));
  if (actual === 'admin') return wanted.includes('admin');
  // Vérifier les capability columns si elles existent, sinon fallback au rôle
  const nonAdminWanted = wanted.filter(r => r !== 'admin');
  if (!nonAdminWanted.length) return false;
  if (profileHasCapabilityColumns(profile)) {
    if (nonAdminWanted.includes('les deux') && hasCapability(profile, 'est_livreur') && hasCapability(profile, 'est_expediteur')) return true;
    if (nonAdminWanted.includes('livreur') && hasCapability(profile, 'est_livreur')) return true;
    if (nonAdminWanted.includes('expediteur') && hasCapability(profile, 'est_expediteur')) return true;
  }
  // Fallback au champ role (compat arrière — capability columns pas toujours settées)
  return nonAdminWanted.some(r => r === actual);
}
