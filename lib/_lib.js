...
function deliveryEligibility(profile, livraison) {
  ...
  // Filtrage par taille colis : XL/électroménager exige camionnette
  if (taille === 'xl' && !['motor'].includes(mode)) {
    return { allowed: false, reason: 'XL / électroménager : camionnette requise', mode, routeKm };
  }
  if (taille === 'gros' && ['foot', 'bike', 'scooter'].includes(mode)) {
    return { allowed: false, reason: 'Gros colis : véhicule motorisé requis', mode, routeKm };
  }
