...
async function uploadProofPhoto(sbUrl, sbKey, livraisonId, dataUrl, bucket) {
  ...
  const upload = await fetch(`${sbUrl}/storage/v1/object/${targetBucket}/${objectPath}`, {
    method: 'POST',
    headers: {
      apikey: sbKey,
...
function estimateRouteKm(from, to) {
  ...
  if (c1 && c2) {
    const km = Math.round(haversineKm(c1[0], c1[1], c2[0], c2[1]));
    // Ajustement routier : +30% pour compenser les routes réelles vs vol d'oiseau
    return Math.max(2, Math.round(km * 1.3));
  }
  // Fallback prudent : 50 km au lieu de 200 (évite les prix gonflés)
  return 50;
}
