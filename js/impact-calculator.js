/**
 * PorteàPorte — Calculateur d'impact public
 * Montre ce que livreur/communauté/etc reçoivent pour chaque dollar
 * Branche sur platform_settings (transparent et live)
 */
(function() {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .pap-calc::before { content: '💎'; position: absolute; top: -15px; right: -15px; font-size: 150px; opacity: .04; }
      ...
      .pap-calc .comparison .compare-row { display: flex; justify-content: space-between; padding: 6px 0; font-size: .88rem; color: #d8dde6; }
      .pap-calc .comparison strong { color: #fff; }
  ...
  function render(container) {
    ...
    container.innerHTML = `
      <div class="pap-calc">
        <h3>💎 Où va chaque dollar que tu paies ?</h3>
        <div class="subtitle">100% transparent — tu vois exactement où va ton argent. Ajuste le curseur pour simuler n'importe quel montant.</div>

        <div class="preset-row">
          <button class="preset-btn" onclick="window._papCalcSet(${container.id ? `'${container.id}'` : 'this.closest(\\'.pap-calc\\').parentNode.id'}, 5)">5 $ (mini)</button>
          <button class="preset-btn active" onclick="window._papCalcSet(this, 15)">15 $ (moyen)</button>
          <button class="preset-btn" onclick="window._papCalcSet(this, 25)">25 $ (régional)</button>
          <button class="preset-btn" onclick="window._papCalcSet(this, 50)">50 $ (gros)</button>
          <button class="preset-btn" onclick="window._papCalcSet(this, 100)">100 $ (XL)</button>
        </div>
        ...
        <div class="comparison">
          <h4>📊 Comparaison vs compétiteurs (pour le livreur)</h4>
          <div class="comparison-content"></div>
        </div>

        <div class="footer-note">
          📊 Pourcentages mis à jour en temps réel · <a href="/transparence.html">Voir le détail complet</a> · <a href="/organismes.html">Où va le 5% communauté ?</a>
        </div>
      </div>
    `;
  ...
  function renderBreakdown(container, amount) {
    ...
    const posts = [
      ...
      { icon: '🏛️', name: 'Opérations (support, équipe)', pct: settings.pct_operations },
      { icon: '💰', name: 'Profit net (équipe québécoise)', pct: settings.pct_profit },
      { icon: '💳', name: 'Frais Stripe (incompressible)', pct: settings.pct_stripe, isStripe: true }
    ];
      ...
      return `
        <div class="row ${cls}">
          ...
          <div class="info">
            ...
            <div class="pct">${p.pct.toFixed(1)}%${p.isStripe ? ' du brut' : ' du net'}</div>
          </div>
    ...
    container.querySelector('.comparison-content').innerHTML = `
      <div class="compare-row"><span>🟢 PorteàPorte (60% net)</span> <strong style="color:#b8f53e">${livreurPaP.toFixed(2)} $</strong></div>
      <div class="compare-row"><span>🔴 Uber Eats (~32%)</span> <strong>${livreurUber.toFixed(2)} $</strong></div>
      <div class="compare-row"><span>🔴 DoorDash (~30%)</span> <strong>${livreurDoorDash.toFixed(2)} $</strong></div>
      <div class="compare-row" style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08)"><span style="color:#7dffc1">✨ Différence en faveur du livreur PàP</span> <strong style="color:#7dffc1">+${(livreurPaP - livreurUber).toFixed(2)} $ par livraison</strong></div>
    `;
  }
